...
static void nrt_init(volatile void *ctx_ptr, volatile hal_pin_inst_t *pin_ptr) {
  ...
  struct encfb_pin_ctx_t *pins = (struct encfb_pin_ctx_t *)pin_ptr;
  ...
  //TIM CH1 A
  GPIO_InitStruct.GPIO_Pin   = FB1_A_PIN;
  GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_AF;
  GPIO_InitStruct.GPIO_Speed = GPIO_Speed_100MHz;
  GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStruct.GPIO_PuPd  = GPIO_PuPd_UP;
  GPIO_Init(FB1_A_PORT, &GPIO_InitStruct);
  ...
  DMA_InitStruct.DMA_PeripheralBaseAddr = (uint32_t)&FB1_ENC_TIM->CCR1;
  ...
  FB1_ENC_TIM->CR1 &= ~TIM_CR1_CEN;
  FB1_ENC_TIM->CCMR1 = TIM_CCMR1_CC1S_0;                                // cc1 input ti1
  FB1_ENC_TIM->CCER  = TIM_CCER_CC1E | TIM_CCER_CC1P | TIM_CCER_CC1NP;  // cc1 en, rising edge, falling edge
  FB1_ENC_TIM->ARR   = 65535;
  FB1_ENC_TIM->DIER  = TIM_DIER_CC1DE;  // enable cc1 dma reeuest
  ...
  GPIO_SetBits(GPIOE, GPIO_Pin_14);  //tx enable
...
static void rt_func(float period, volatile void *ctx_ptr, volatile hal_pin_inst_t *pin_ptr) {
  ...
  uint32_t count = ARRAY_SIZE(tim_data) - DMA2_Stream3->NDTR;
  PIN(dma)       = count;
...
*/
  ...
  FB1_ENC_TIM->CNT  = 0;
  FB1_ENC_TIM->CCR1 = 0;
  FB1_ENC_TIM->CR1 |= TIM_CR1_CEN;  // enable tim

  //send request, 1/(42e6/32)*11 = 8.4uS
  SPI2->DR = PIN(req_len);
  //start DMA
  DMA_Cmd(DMA2_Stream3, DISABLE);
  DMA_ClearFlag(DMA2_Stream3, DMA_FLAG_TCIF0);
...
hal_comp_t encfb_comp_struct = {
    ...
    .frt_stop  = 0,
    .ctx_size  = 0,
    .pin_count = sizeof(struct encfb_pin_ctx_t) / sizeof(struct hal_pin_inst_t),
};
